...
const PERMISSIONS_LIST = [
    'MANAGE_DASHBOARD', 'MANAGE_PRODUCTS', 'MANAGE_ORDERS', 'MANAGE_USERS', 'MANAGE_PAYMENTS', 'MANAGE_SETTINGS', 'MANAGE_ADMIN', 'MANAGE_SIDEBAR', 'MANAGE_PACKAGES'
];
                        ...
                        <div className="bg-(--bg-box) rounded-2xl border border-(--bs-border) p-8 shadow-sm">
                            ...
                            <div className="space-y-2">
                                {PERMISSIONS_LIST.map(permission => (
                                    <label
                                        key={permission}
                                        className="flex items-center gap-3 p-3 rounded-lg hover:bg-(--bg-hover) transition-colors cursor-pointer group"
                                    >
                                        <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${formData.permissions.includes(permission)
                                            ? 'bg-blue-600 border-blue-600 text-white'
                                            : 'border-gray-300 group-hover:border-blue-400'
                                            }`}>
                                            {formData.permissions.includes(permission) && <FiCheckCircle size={14} />}
                                            <input
                                                type="checkbox"
                                        ...
                                        <span className={`text-sm font-medium transition-colors ${formData.permissions.includes(permission) ? 'text-blue-700' : 'text-gray-600'
                                            }`}>
