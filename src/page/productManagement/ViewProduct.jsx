...
const ViewProduct = () => {
  ...
  if (loading) return <div className="p-10 text-center text-(--text-second)">Loading product details...</div>;
    ...
    <div className="pb-20 relative min-h-screen bg-[var(--bg-main)]">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--text-second)] text-sm mt-0.5 flex items-center gap-2">
              ID: <span className="font-mono text-(--text-second)">{product._id}</span>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <div className="bg-[var(--bg-box)] rounded-2xl p-4 shadow-sm border border-[var(--bs-border)]">
            <div className="aspect-[4/3] w-full rounded-xl overflow-hidden bg-(--bg-main) mb-4 border border-[var(--bs-border)] relative items-center flex justify-center">
          ...
          {product.package && (
            <div className="card-box2 rounded-2xl p-6 relative overflow-hidden shadow-sm">
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-3 text-[var(--text-second)]">
                  <div className="p-2 bg-(--bg-box)/50 rounded-lg backdrop-blur-sm">
                    <FiPackage size={20} />
                  </div>
                ...
                {(typeof product.package === 'object' && product.package.price) && (
                  <p className="text-sm font-medium text-[var(--text-second)]">Package Price: ₹{product.package.price}</p>
                )}
              </div>
          ...
          <div className="bg-[var(--bg-box)] rounded-2xl p-6 shadow-sm border border-[var(--bs-border)]">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-(--bs-border)">
              ...
              <div className="flex justify-between items-center py-2 border-b border-(--bs-border)">
                ...
                <span className="font-mono text-xs bg-[var(--bg-main)] px-2 py-1 rounded text-[var(--text-second)]">{product.sku || 'N/A'}</span>
              </div>
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-[var(--bg-box)] p-5 rounded-2xl border border-[var(--bs-border)] shadow-sm flex flex-col justify-between">
              ...
              <div className="flex gap-3 items-end">
                <div><span className="text-xs text-[var(--text-second)] block">PV</span><span className="font-bold text-orange-700">{product.pv || 0}</span></div>
                <div className="w-px h-8 bg-[var(--bs-border)]"></div>
                <div><span className="text-xs text-[var(--text-second)] block">BV</span><span className="font-bold text-orange-700">{product.bv || product.bp || 0}</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[var(--bg-box)] rounded-2xl p-6 shadow-sm border border-[var(--bs-border)]">
            ...
            <div className="prose prose-sm max-w-none text-[var(--text-second)] leading-relaxed"
              dangerouslySetInnerHTML={{ __html: product.description || "<p className='italic text-(--text-third)'>No description provided.</p>" }}
            />
          </div>
            ...
            <div className="bg-[var(--bg-box)] rounded-2xl p-6 shadow-sm border border-[var(--bs-border)]">
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {product.keyFeatures.map((feat, i) => (
                      <div key={i} className="flex items-start gap-3 p-3 bg-[var(--bg-main)] rounded-xl border border-[var(--bs-border)]">
                        <div className="mt-0.5 min-w-[20px] h-[20px] rounded-full bg-[var(--bg-box)] border border-green-200 flex items-center justify-center text-green-600 shadow-sm">
                          <FiCheck size={12} />
                        </div>
                        <p className="text-sm text-[var(--text-second)] leading-snug">{feat}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {product.specifications?.length > 0 && (
                <div>
                  ...
                  <div className="bg-[var(--bg-box)] rounded-xl border border-[var(--bs-border)] overflow-hidden">
                    {product.specifications.map((spec, i) => (
                      <div key={i} className="flex flex-col sm:flex-row border-b last:border-0 border-[var(--bs-border)] hover:bg-[var(--bg-main)] transition-colors">
                        <div className="sm:w-1/3 px-4 py-3 bg-[var(--bg-main)] text-sm font-medium text-[var(--text-second)] border-r border-[var(--bs-border)]">
                          {spec.label}
                        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {product.colors.map((col, i) => (
                      <div key={i} className="flex items-center gap-3 pr-4 pl-1 py-1 rounded-full border border-[var(--bs-border)] bg-[var(--bg-box)] shadow-sm transition-colors cursor-default">
                        <div
                          className="w-8 h-8 rounded-full border border-[var(--bs-border)] shadow-inner"
                          style={{ backgroundColor: col.hex || '#ccc' }}
                        />
                        <span className="text-sm font-medium text-[var(--text-second)]">{col.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.sizes.map((size, i) => (
                      <div key={i} className="min-w-[40px] h-[40px] px-3 flex items-center justify-center bg-[var(--bg-main)] border border-[var(--bs-border)] rounded-lg text-sm font-semibold text-[var(--text-second)] shadow-sm">
        ...
        <div className="bg-[var(--bg-box)] rounded-2xl p-6 shadow-sm border border-[var(--bs-border)]">
          ...
          <div className="bg-[var(--bg-box)] p-4 rounded-xl border border-[var(--bs-border)] shadow-sm hover:shadow-md transition-shadow cursor-pointer max-w-xl">
            <div className="flex items-center gap-2 mb-1">
              <div className="w-6 h-6 rounded-full bg-[var(--bg-main)] p-1">
                <FiGlobe className="w-full h-full text-[var(--icon-color)]" />
              </div>
              <span className="text-xs text-[var(--text-second)]">yoursite.com</span>
              <span className="text-xs text-[var(--text-third)]">› product</span>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {product.metaKeywords?.length > 0 ? product.metaKeywords.map((k, i) => (
              <span key={i} className="text-xs bg-[var(--bg-main)] text-[var(--text-second)] px-2 py-1 rounded-md border border-[var(--bs-border)]">#{k}</span>
            )) : <span className="text-xs text-[var(--text-third)] italic">No keywords configured</span>}
          </div>
        </div>
